                    ...
                    <form action="{{ route('tickets.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-6">
                            ...
                            <textarea id="description" name="description" rows="6"
                                      ...
                                      placeholder="Something is wrong, here's the error message: ..." required>{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <div class="flex gap-4 flex-wrap">
                                @foreach($labels as $label)
                                    <div class="flex items-center">
                                        ...
                                        <label for="label_{{ $label->id }}" class="ml-2 text-sm font-medium text-gray-900">
                                            {{ $label->name }}
                                        </label>
                                    </div>
                                @endforeach
                                ...
                                @foreach($categories as $category)
                                    <div class="flex items-center">
                                        <input id="category_{{ $category->id }}" type="checkbox" value="{{ $category->id }}" name="categories[]"
                                               ...
                                               class="w-4 h-4 text-purple-600 bg-gray-100 border-gray-300 rounded focus:ring-purple-500">
                                        <label for="category_{{ $category->id }}" class="ml-2 text-sm font-medium text-gray-900">
                                            {{ $category->name }}
                                        </label>
                                    </div>
                                @endforeach
                        ...
                        {{-- Priority --}}
                        <div class="mb-6">
                            <label for="priority_id" class="block mb-2 text-sm font-medium text-gray-900">
                                Priority
                            </label>
